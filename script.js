...
async function loadProfileData() {
    try {
        ...
        if (data.links) {
            ...
            if (linksGrid) {
                linksGrid.innerHTML = data.links.map(function(link) {
                    var tag = link.url ? 'a href="' + escapeHtml(link.url) + '" target="_blank" rel="noopener noreferrer"' : 'div';
                    return '<' + tag + ' class="link-card">' +
                        (link.icon ? '<div class="link-icon">' + escapeHtml(link.icon) + '</div>' : '') +
        ...
        if (educationTimeline && data.education) {
            educationTimeline.innerHTML = data.education.map(function(edu) {
                return '<div class="timeline-item"><div class="timeline-date">' + escapeHtml(edu.startYear + ' - ' + edu.endYear) + '</div>' +
                    '<div class="timeline-title">' + escapeHtml(edu.degree) + '</div>' +
                    '<div class="timeline-company">' + escapeHtml(edu.school) + '</div>' +
                    (edu.details ? '<div class="timeline-description">' + escapeHtml(edu.details) + '</div>' : '') + '</div>';
            }).join('');
        }
